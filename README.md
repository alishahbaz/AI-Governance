...
```mermaid
flowchart TD
  A["Step 1: Vocabulary and Failures"] --> B["Step 2: Identify and Categorize Risks"]
  B --> C["Step 3: Ethics by Design"]
  C --> D["Step 4: Organizational Governance"]
  D --> E["Step 5: Global Regulations"]
  E --> F["Step 6: Change Management and Digital Literacy"]
  F --> G["Trustworthy AI"]
```
...
```mermaid
flowchart TD
  A["AI System"] --> B["Governance"]
  B --> C["Vocabulary"]
  B --> D["Risk Management"]
  B --> E["Technical Safeguards"]
  B --> F["Organizational Roles"]
  B --> G["Legal Compliance"]
  B --> H["Human Change"]
  C --> C1["Policies"]
  C --> C2["Regulations"]
  C --> C3["Standards"]
  ...
  D --> D3["Misuse"]
  D --> D4["Deepfakes"]
  E --> E1["Ethics by Design"]
  E --> E2["Data Mitigations"]
  ...
  G --> G1["EU AI Act"]
  G --> G2["US NIST AI RMF"]
  G --> G3["International Standards"]
  H --> H1["Reskilling"]
  H --> H2["AI Literacy"]
  H --> H3["User Skepticism"]
```
...
```mermaid
flowchart TD
  R["AI Risk"] --> A["Algorithm"]
  R --> D["Data"]
  R --> M["Misuse"]

  A --> A1["Black-box decisions"]
  A --> A2["Unexpected behavior"]
  D --> D1["Bias"]
  D --> D2["Privacy issues"]
  M --> M1["Fraud"]
  M --> M2["Deepfakes"]
...
```mermaid
flowchart TD
  M["Minimal Risk"] --> L["Limited Risk"]
  L --> H["High Risk"]
  H --> U["Unacceptable Risk"]
```
